...
/*
// For REG_RSP/REG_RIP/... in glibc's ucontext.h on linux/amd64; must be
// defined before the first libc include.
#define _GNU_SOURCE
...
static void wz_gs_handler(int sig, siginfo_t *info, void *uc) {
	...
	for (i = 0; i < WZ_GS_MAX_REGIONS; i++) {
		...
		// The fault address is inside a registered region. If the thread is
...
func init() {
	...
	experimental.SetGuardFaultHandler(handler{})
}

// Supported returns true when the fault handler was successfully installed
// and unchecked memory access will therefore be effective when paired with
// a LinearMemory that registers its regions here (e.g. experimental/guardpage).
func Supported() bool { return installed }

// RegisterRegion records a reserved address range [base, end) so a fault
// inside it, on a thread executing a registered call stack, is recognized
// as a guest out-of-bounds access rather than an unrelated crash. Returns
// false if the handler's fixed-size region table is full or the handler
// failed to install, in which case the region must not be used to back
// checkless code.
//
// Called directly by a LinearMemory/MemoryAllocator implementation (such as
// experimental/guardpage) — wazero itself is not involved in region
// tracking, only in resolving which live call a fault occurred on (see
// RegisterCallStack).
func RegisterRegion(base, end uintptr) bool {
	if !installed {
		return false
	}
	mu.Lock()
...
// UnregisterRegion removes a previously registered region.
...
// handler implements experimental.GuardFaultHandler.
type handler struct{}

// RegisterCallStack implements experimental.GuardFaultHandler.
func (handler) RegisterCallStack(lo, hi, execCtx, exitSeq uintptr) bool {
	mu.Lock()
	defer mu.Unlock()
	n := len(stackFree)
	if n == 0 {
...
// UnregisterCallStack implements experimental.GuardFaultHandler.
func (handler) UnregisterCallStack(lo uintptr) {
	mu.Lock()
	defer mu.Unlock()
