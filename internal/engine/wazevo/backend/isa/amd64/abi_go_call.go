...
// here with the execution context pointer in rax and the faulting program
// counter in rcx. The sequence records the trap exactly like an inline
// lowerExitWithCode (exit code, RSP/RBP for unwinding, trap address for
// source mapping) and then exits the wasm execution.
