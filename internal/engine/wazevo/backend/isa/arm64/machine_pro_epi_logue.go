...
//
// A registered experimental.GuardFaultHandler redirects a faulting thread
// here with the execution context pointer in x0 and the faulting program
// counter in x1. The sequence records the trap exactly like an inline
// lowerExitWithCode (exit code, stack pointer for unwinding, trap address
// for source mapping) and then exits the wasm execution.
