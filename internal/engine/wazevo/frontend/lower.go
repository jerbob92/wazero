...
func (c *Compiler) memOpSetup(baseAddr ssa.Value, constOffset, operationSizeInBytes uint64) (address ssa.Value) {
	...
	// With memory access checks disabled (experimental.WithUncheckedMemoryAccess),
	// the memory instance guarantees any u32 base + u32 static offset +
	// access size either lands inside allocated memory or faults in a way
	// recoverable back to the standard wasm out-of-bounds trap by the
	// registered experimental.GuardFaultHandler, so an explicit check is
	// unnecessary.
	if c.memoryUnchecked {
...
// markGuestMemoryLoad marks a guest linear-memory load as trapping when
// memory access checks are disabled (experimental.WithUncheckedMemoryAccess):
// the access itself then carries the wasm out-of-bounds trap semantics and
// must not be dead-code-eliminated even if its result is unused.
