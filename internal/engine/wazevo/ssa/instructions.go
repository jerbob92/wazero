...
// memory accesses compiled without explicit bounds checks (see
// experimental.WithUncheckedMemoryAccess), where the hardware fault raised
// by the access itself implements the wasm out-of-bounds trap and the
// access therefore must not be dead-code-eliminated.
